...
export function RelatedCalcs({ currentSlug }: { currentSlug: string }) {
  ...
  return (
    <aside className="mt-8 rounded-md border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900">
      ...
      <ul className="grid gap-3 md:grid-cols-3">
        {related.map((t) => (
          <li key={t.slug}>
            <Link
              href={`/${t.slug}`}
              className="block rounded-md border border-slate-200 bg-white p-3 text-sm shadow-sm transition hover:border-brand-500 hover:shadow dark:border-slate-700 dark:bg-slate-950"
            >
