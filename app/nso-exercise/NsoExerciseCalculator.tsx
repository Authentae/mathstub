...
export function NsoExerciseCalculator() {
  ...
  const fmv = toNumberOrZero(form.fmvUsd);
  const shares = toNumberOrZero(form.shares);
  const bargainElement = Math.max(0, (fmv - strike) * shares);
  const cashToExercise = strike * shares;

  const result: RsuShortfallResult | { error: string } = useMemo(() => {
    if (bargainElement === 0) {
      return { error: 'Enter strike, FMV, and shares — bargain element must be positive.' };
    }
    try {
      const input: RsuShortfallInput = {
        taxYear: form.taxYear,
        filingStatus: form.filingStatus,
        vestGrossUsd: bargainElement,
  ...
  return (
    <div className="space-y-6">
      <form className="grid gap-4 rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 md:grid-cols-2">
        <Field label="Strike price ($/share)">
          <input
            ...
            onChange={(e) => update('strikeUsd', e.target.value)}
            className={inputCls}
          />
        </Field>
        <Field label="FMV at exercise ($/share)">
          <input
            ...
            step="0.01"
            value={form.fmvUsd}
            onChange={(e) => update('fmvUsd', e.target.value)}
            className={inputCls}
          />
        </Field>
      ...
      <div className="grid gap-3 rounded-md border border-blue-200 bg-blue-50 p-4 text-sm dark:border-blue-900 dark:bg-blue-950 md:grid-cols-2">
        <div>
          <p className="font-semibold text-blue-900 dark:text-blue-200">Bargain element</p>
          <p className="text-2xl font-bold text-blue-700 dark:text-blue-100">{usd.format(bargainElement)}</p>
          <p className="mt-1 text-xs text-blue-700 dark:text-blue-300">(FMV − strike) × shares — taxed as ordinary income</p>
        </div>
        <div>
          <p className="font-semibold text-blue-900 dark:text-blue-200">Cash to exercise</p>
          <p className="text-2xl font-bold text-blue-700 dark:text-blue-100">{usd.format(cashToExercise)}</p>
          <p className="mt-1 text-xs text-blue-700 dark:text-blue-300">strike × shares — what you write a check for to buy the shares</p>
        </div>
      </div>
      ...
      {'error' in result ? (
        ...
        <Result result={result} bargainElement={bargainElement} />
      )}
    </div>
  );
}
...
function Result({ result, bargainElement }: { result: RsuShortfallResult; bargainElement: number }) {
  const r = result;
    ...
    <div className="space-y-6">
      ...
      <details className="rounded-md border border-gray-200 bg-white p-4 text-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          <Row k="Bargain element (ordinary income)" v={usd.format(bargainElement)} />
